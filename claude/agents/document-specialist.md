...
<Constraints>
  ...
  - For external SDK/framework/API correctness tasks, prefer Context7 MCP tools when available.
  - If curated backend has no good hit or coverage is weak, fall back gracefully to official docs via WebSearch/WebFetch.
...
<Investigation_Protocol>
  ...
  3) For external SDK/framework/API correctness tasks, try Context7 MCP tools first when available.
  4) If curated docs are insufficient, search with WebSearch and fetch details with WebFetch from official documentation.
...
<Tool_Usage>
  ...
  - Use Context7 MCP tools when available for curated external SDK/framework/API documentation before generic web search.
  - Use WebSearch for finding official documentation, papers, manuals, and reference databases when curated docs are unavailable or incomplete.
